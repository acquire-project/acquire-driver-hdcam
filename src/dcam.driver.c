...
shutdown_(struct Driver* self_)
{
    ...
    self->inner = 0;
    lib_close((struct lib*)self->hmodule);
    free(self);
    return ecode;
Error:
    return Device_Err;
...
driver_load()
{
    ...
    *self = (struct Loader){
    ...
    EXPECT(self->inner = acquire_driver_init_v0(reporter),
...
acquire_export int
unit_test__reset_driver_and_replace_camera()
{
    struct Driver* driver = NULL;
    struct Dcam4Driver* dcam_driver = NULL;

    struct Device* device = NULL;
    struct Camera* camera = NULL;
    struct Dcam4Camera *dcam_camera = NULL, *dcam_camera2 = NULL;

    driver = driver_load();

    CHECK(driver);
    dcam_driver = containerof(driver, struct Dcam4Driver, driver);

    const uint32_t n_devices = driver->device_count(driver);
    CHECK(n_devices > 0);

    // ape driver_open_device here
    CHECK(Device_Ok == driver->open(driver, 0, &device));
    CHECK(device);
    CHECK(Device_Ok == driver->describe(driver, &device->identifier, 0));
    device->driver = driver;

    dcam_camera = containerof(device, struct Dcam4Camera, camera.device);
    CHECK(dcam_camera);
    camera = &dcam_camera->camera;

    // camera_start
    CHECK(Device_Ok == aq_dcam_start(camera));
    camera->state = DeviceState_Running;

    clock_sleep_ms(0, 1000);

    // camera_stop
    CHECK(Device_Ok == aq_dcam_stop(camera));
    camera->state = DeviceState_Armed;

    // reset the driver and replace the camera
    dcam_camera2 = reset_driver_and_replace_camera(dcam_camera);
    CHECK(dcam_camera = dcam_camera2);
    ...
    // cleanup
    CHECK(Device_Ok == driver->close(driver, device));
    device = NULL; // driver_close_device frees the device

    CHECK(Device_Ok == driver->shutdown(driver));
    driver = NULL; // driver_shutdown frees the driver
    return 1;
Error:
    if (device)
        free(device);
